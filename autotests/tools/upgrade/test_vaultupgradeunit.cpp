...
protected:
    void SetUp() override {
        ...
        QDir().mkpath(testDir + "/.local/share/applications/vault_encrypted");
        QDir().mkpath(testDir + "/.config/Vault/vault_encrypted");
...
TEST_F(TestVaultUpgradeUnit, initialize_WithOldVault)
{
    ...
    QString oldConfigPath = testDir + "/.local/share/applications/vault_encrypted/cryfs.config";
    QDir().mkpath(QFileInfo(oldConfigPath).absolutePath());
    ...
    QString newConfigPath = testDir + "/.config/Vault/vault_encrypted/cryfs.config";
